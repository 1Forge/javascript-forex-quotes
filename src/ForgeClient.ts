...
class ForgeClient {
  ...
  constructor(
    ...
    private beta: boolean = false,
    settings?: ForgeClientSettings,
    private temporary: boolean = true,
  ) {
    this.restClient = new RestClient(apiKey, beta, settings && settings.rest);
    this.socketClient = new SocketClient(apiKey, beta);
  }
